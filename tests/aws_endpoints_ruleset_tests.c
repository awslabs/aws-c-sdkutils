...
static int read_file_contents(struct aws_byte_buf *out_buf, struct aws_allocator *alloc, const char *filename) {
    ...
    ASSERT_INT_EQUALS(aws_byte_buf_init(out_buf, alloc, file_size), AWS_OP_SUCCESS);
    size_t read = fread(out_buf->buffer, 1, file_size, fp);
    fclose(fp);
...
static int s_test_parse_ruleset_from_string(struct aws_allocator *allocator, void *ctx) {
    ...
    ASSERT_INT_EQUALS(read_file_contents(&buf, allocator, "sample_ruleset.json"), AWS_OP_SUCCESS);
