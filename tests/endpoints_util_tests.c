...
AWS_TEST_CASE(endpoints_template_replace, s_test_template_replace)
static int s_test_template_replace(struct aws_allocator *allocator, void *ctx) {
    (void)ctx;
    ...
    ASSERT_SUCCESS(
        aws_templated_string_replace_escaped(allocator, aws_byte_cursor_from_c_str("another long {{ string }"), &buf4));
...
static int s_test_json_template_replace(struct aws_allocator *allocator, void *ctx) {
    ...
    struct aws_byte_buf buf1;
    ASSERT_SUCCESS(aws_json_templated_strings_replace_escaped(
        allocator, aws_byte_cursor_from_c_str("{\"foo\":{\"bar\":\"{{}}\"}}"), &buf1));
