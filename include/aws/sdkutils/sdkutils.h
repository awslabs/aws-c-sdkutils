...
enum aws_sdkutils_errors {
  AWS_ERROR_SDKUTILS_BEGIN_RANGE =
      AWS_ERROR_ENUM_BEGIN_RANGE(AWS_C_SDKUTILS_PACKAGE_ID),

  AWS_ERROR_SDKUTILS_END_RANGE =
      AWS_ERROR_ENUM_END_RANGE(AWS_C_SDKUTILS_PACKAGE_ID)
};

enum aws_sdkutils_log_subject {
  AWS_LS_SDKUTILS_GENERAL =
      AWS_LOG_SUBJECT_BEGIN_RANGE(AWS_C_SDKUTILS_PACKAGE_ID),
...
AWS_SDKUTILS_API void
aws_sdkutils_library_init(struct aws_allocator *allocator);
